...
export class EventdetailsPage implements OnInit {
  ...
  constructor(public formBuilder: FormBuilder,
      ...
      private apiService:ApiService){
        this.selectedEventId = this.activatedRoute.snapshot.params.id
        console.log(this.selectedEventId);
        this.loadEvent(true,this.selectedEventId) ;
      }

  ngOnInit() {
    this.upadteEventForm = this.formBuilder.group({
  ...
  validation_messages = {
    'eventName': [
      { type: 'required', message: 'Event Name required' }
    ],
    'startDate': [
    ...
    'startTime': [
      { type: 'required', message: 'Start Time required' }
    ],
    'endTime': [
      { type: 'required', message: 'End Time required' }
    ],
    'eventType': [
      { type: 'required', message: 'Event Type required' }
    ]
  ...
  ionViewDidEnter(){
    console.log('EventdetailsPage::ionViewDidEnter() | method called');
    this.loadEvent(false,this.selectedEventId);
  }

  loadEvent(refresh = false, eventId:any, refresher?) {
    this.apiService.getEvents(refresh).subscribe(res => {
      if(res!=null){
        var singlEvent=res[0].Events.filter(id=>id.id==eventId);
        this.event=singlEvent[0];
        this.event_desc=this.event.event_desc;
        this.startDate=this.event.startDate;
        this.endDate=this.event.endDate;
        this.eventType=this.event.calendar_event_type;
        console.log("singleevent"+JSON.stringify(this.event.event_desc));
      } 
      else{
        this.event = null;
      }
   ...
   onSubmit(values){
     var updateFormData={'id':this.selectedEventId};
     var updateEventValues=JSON.stringify(values);
      var startDate=moment(values.startDate).format('MM/DD/YYYY');
      var endDate=moment(values.endDate).format('MM/DD/YYYY');
      var startTime=moment(values.startTime).format('hh:mm A');
      var endTime=moment(values.endTime).format('hh:mm A');
      updateFormData['event_desc']=values.eventName;
      updateFormData['start_date']=startDate;
      updateFormData['end_date']=endDate;
      updateFormData['start_time']=startTime;
      updateFormData['end_time']=endTime;
      updateFormData['event_type']=values.eventType;
      this.apiService.updateEvent(updateFormData).subscribe(res=>{
        if(res.status_code==1){
          this.alertService.presentToast('Event updated successfully.');
          this.router.navigate(['tabs/calendar']);
        }
        else{
          this.alertService.presentToast(res.error_message); 
        }
      },error=>{
      ...
      console.log("startDate"+startDate+startTime+endTime);
     console.log("updateEventValues"+JSON.stringify(values));
   }
